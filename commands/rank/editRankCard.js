...
new Command({
  ...
  run: async (ctx) => {
    ...
    logInfo(
      `Creating a rank editor session with id ${sessionID} for user ${ctx.user.id}`
    );
    ...
    logSuccess(
      `Created a rank editor session with id ${sessionID} for user ${ctx.user.id}`
    );

    const buttonRow = new ActionRowBuilder().addComponents([
      new ButtonBuilder()
        .setLabel("Edit Rank Card")
        .setURL(`${config.discord.leaderboardURL}/rankCard/${session._id}`)
