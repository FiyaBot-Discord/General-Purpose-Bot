...
new Command({
  ...
  run: async (ctx) => {
    ...
    if (!userRankDetails) {
      ...
      if (user.id === ctx.member.id) {
        return ctx.editReply({
          content: "You have not sent any messages yet!",
          ephemeral: true,
        });
      } else {
        return ctx.editReply({
    ...
    const progressBarColor =
      userRankDetails.rankCard.progressBarColor || userRankDetails.displayHex;
    ...
    logInfo(
      `Creating url with params to get rank card gif for user ${user.id}`
    );
    const url = new URL(`${config.discord.apiBaseURL}/rankcard/rankCardGif`);
