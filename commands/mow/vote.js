  ...
  run: async (ctx) => {
    try {
      ...
      return ctx.editReply({
        content: "This command is disabled for now",
        ephemeral: true,
      });
      ...
      const user = userArg
        ? await ctx.guild.members.fetch(userArg.id)
        : ctx.member;

      // Get voters xp profile
      const voterXPProfile = await xpUser.findById(
        `${ctx.user.id}_${ctx.guild.id}`
      );

      if (user.id === ctx.member.id) {
        voterXPProfile.mowPoints = voterXPProfile.mowPoints
          ? voterXPProfile.mowPoints - 5
          : -5;
        try {
          ...
          console.error(err);
          ctx.editReply({
            content: "An error occurred, try again in a bit",
            ephemeral: true,
          });
        }
      }
      ...
      if (voterXPProfile.votesLeft === 0) {
        const embed = new MessageEmbed()
          .setTitle("Nuh uh")
          .setDescription(
            "<a:finger_wave:1279526590811865120> You have used up your votes for today"
          )
          .setColor(config.colors.primary)
          .setTimestamp();
        return ctx.editReply({ embeds: [embed] });
      ...
      if (
        voterXPProfile.votedMembers &&
        voterXPProfile.votedMembers.includes(user.id)
      ) {
        const embed = new MessageEmbed()
          .setTitle("Nuh uh")
          .setDescription(
            "<a:finger_wave:1279526590811865120> You have already voted for this person today"
          )
          .setColor(config.colors.primary)
          .setTimestamp();
        return ctx.editReply({ embeds: [embed] });
      ...
      if (!userXPProfile) {
        return ctx.editReply({
          content: "This person needs to send a message first",
          ephemeral: true,
        });
      }

      // Add 10 mowpoints to the user
      userXPProfile.mowPoints = userXPProfile.mowPoints
        ? userXPProfile.mowPoints + 10
        : 10;
      ...
      voterXPProfile.votedMembers = voterXPProfile.votedMembers
        ? voterXPProfile.votedMembers.concat(user.id)
        : [user.id];
      // Remove 1 vote from the voter
      voterXPProfile.votesLeft = voterXPProfile.votesLeft
        ? voterXPProfile.votesLeft - 1
        : config.discord.voteCount - 1;
      ...
      const embed = new MessageEmbed()
        .setTitle("Wooo")
        .setDescription(
          `You're so kind <a:pleadcry:1279526558855598184>. <@${user.id}> has received 10 points.`
        )
        .setColor(config.colors.primary)
        .setTimestamp();
      ...
      return ctx.channel.send({
        content: "An error occurred while trying to vote for the user",
      });
    }
  },
});
