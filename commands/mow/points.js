    ...
    try {
      ...
      if (!userXPProfile || !userXPProfile.mowPoints) {
        if (user.id !== ctx.user.id) {
          return ctx.editReply({
            content: "This person doesnt have any points :sob:",
          });
        } else {
          return ctx.editReply({
            content: "You dont have any points. Stop slacking",
          });
        }
      }
      ...
      const embed = new MessageEmbed()
        .setTitle("Points")
        .setDescription(
          `<:purple_candy:1279057588822216835> You've got ${userXPProfile.mowPoints.toFixed(
            2
          )} points! <:cyan_candy:1279057698482290778>`
        )
        .setImage(
          "https://media.tenor.com/x5jwK4cZEnsAAAAM/pepe-hype-hands-up.gif"
        )
      ...
      if (user.id !== ctx.user.id) {
        embed.setDescription(
          `<:purple_candy:1279057588822216835> <@${
            user.id
          }> has ${userXPProfile.mowPoints.toFixed(
            2
          )} points! <:cyan_candy:1279057698482290778>`
        );
      }
      ...
      if (user.id === ctx.user.id && userXPProfile.mowPoints < 0) {
        setTimeout(async () => {
          embed.setImage(
            "https://media1.tenor.com/m/ASGuOCPGrKEAAAAd/kekw-kek.gif"
          );
          await ctx.editReply({ embeds: [embed] });
          ctx.channel.send({
            content:
              "Oh wait..youre in the negatives... :joy::index_pointing_at_the_viewer:",
          });
        }, 3000);
      }
    } catch (err) {
      ...
      return ctx.channel.send({
        content: "An error occurred while trying to get your points",
      });
    }
  },
});
