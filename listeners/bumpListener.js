    ...
    if (ctx.author.id === "302050872383242240") {
      ...
      if (ctx.embeds[0]?.description?.includes("Bump done!")) {
        try {
          ...
          if (
            bumperDoc.numberOfBumps % 10 === 1 &&
            bumperDoc.numberOfBumps % 100 !== 11
          ) {
            description += "st`";
          } else if (
            bumperDoc.numberOfBumps % 10 === 2 &&
            bumperDoc.numberOfBumps % 100 !== 12
          ) {
            description += "nd`";
          } else if (
            bumperDoc.numberOfBumps % 10 === 3 &&
            bumperDoc.numberOfBumps % 100 !== 13
          ) {
            description += "rd`";
          } else {
            description += "th`";
          ...
          setTimeout(async () => {
            await ctx.channel.send({
              content: `Oi, it's time to bump the server`,
            });
          }, 7200000);
        } catch (err) {
